package topic13_advanced_graphs.practice;

import java.util.PriorityQueue;
import java.util.List;
import java.util.ArrayList;

public class Medium_05_MinCostConnectPoints {

        /**
     * Problem: Min Cost to Connect All Points
     *
     * You are given an array points representing integer coordinates of some points on
     * a 2D-plane, where points[i] = [x_i, y_i].
     *
     * The cost of connecting two points [x_i, y_i] and [x_j, y_j] is the manhattan
     * distance between them: |x_i - x_j| + |y_i - y_j|, where |val| denotes the
     * absolute value of val.
     *
     * Return the minimum cost to make all points connected. All points are connected
     * if there is exactly one simple path between any two points.
     *
     * Example 1:
     * Input: points = [[0,0],[2,2],[3,10],[5,2],[7,0]]
     * Output: 20
     * Explanation:
     * We can connect the points as shown above to get the minimum cost of 20.
     * Notice that there is a unique path between every pair of points.
     *
     * Example 2:
     * Input: points = [[3,12],[-2,5],[-4,1]]
     * Output: 18
     * Constraints:
     *
     *   - 1 <= points.length <= 1000
     *   - -10^6 <= x_i, y_i <= 10^6
     *   - All pairs (x_i, y_i) are distinct.
     */
    public int minCostConnectPoints(int[][] points) {
        // TODO: Implement your solution here.
        return new int[0];
    }

    /* 
     * =========================================================================
     * MENTEE QUESTION REGION
     * =========================================================================
     * Your Question: 
     * 
     */

    /* 
     * =========================================================================
     * MENTOR QUESTION TO MENTEE
     * =========================================================================
     * Mentor Question: У чому різниця між алгоритмами Прима та Крускала для пошуку мінімального кістякового дерева?
     * 
     * Your Answer: 
     * 
     */
}
