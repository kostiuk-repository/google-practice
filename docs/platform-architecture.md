...
Editor додатково використовує `src/services/java21LanguageSupport.ts` і ліниво завантажує `public/java21-api.json`: 4 719 типів та 51 207 members з офіційного Java 21 Javadoc index. Це дає class/import/member autocomplete, quick fix для відсутнього import і локальну перевірку duplicate/unknown imports без backend. Повний symbol resolution залишається відповідальністю `javac`.
