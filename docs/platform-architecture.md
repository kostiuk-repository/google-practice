...
flowchart LR
    ...
    U --> P[Piston-compatible API]
```

Генератор знаходить канонічні файли задач формату `Easy_01_Title.java`, їхні ітерації `Easy_01_01_Title.java`, `_Doc.md` і відповідний тест. У результаті створюються:
...
### Важливе про публічний Piston

Публічний `https://emkc.org/api/v2/piston/execute` більше не гарантує анонімний доступ і може вимагати авторизацію. У Settings можна змінити:

- endpoint;
- версію Java runtime;
- назву й значення auth header;
- compile/run timeouts.

Для стабільного публічного курсу рекомендовано self-hosted Piston або власний тонкий proxy із server-side secret. Не вбудовуйте приватний API token у статичний frontend: будь-який відвідувач GitHub Pages може його прочитати.
